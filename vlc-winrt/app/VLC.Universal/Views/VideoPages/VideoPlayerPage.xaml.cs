...
namespace VLC.UI.Views.VideoPages
{
    public sealed partial class VideoPlayerPage : Page
    {
        ...
        private void PlaceholderInteractionGrid_Loaded(object sender, RoutedEventArgs args)
        {
            ...
            {
                Text = "VR Mode",
                Command = new ActionCommand(() => SwitchToVRView())
            });
        ...
        {
            Debug.WriteLine(Locator.MediaPlaybackViewModel.PlaybackService.CurrentPlaybackMedia.Path);

            SwitchToVRView();

        }

        private void SwitchToVRView()
        {
            VRPlayer.MediaPlayer.FileName = Locator.MediaPlaybackViewModel.PlaybackService.CurrentPlaybackMedia.Path;
            VRPlayer.MediaPlayer.StartPosition = Locator.MediaPlaybackViewModel.PlaybackService.Time / 1000;
            Debug.WriteLine(Locator.MediaPlaybackViewModel.PlaybackService.Time);

            VRPlayer.ViewManagement.SwitchTo3DViewAsync();
